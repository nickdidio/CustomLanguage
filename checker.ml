...
declaration")
  ...
  | SetRef(e1,e2) ->
    type_of_expr e1 >>= 
    arg_of_refType "setref: " >>= fun t ->
    return UnitType
  (* lists *)
  | EmptyList(t) ->
    return (ListType t)
  | Cons(h, t) ->
    type_of_expr h >>= fun s ->
    type_of_expr t >>=
    arg_of_listType "cons: " >>= fun q ->
    if (s=q)
    then return (ListType s)
    else error "cons: type of head and tail do not match"
  | Null(e) ->
    type_of_expr e >>= 
    arg_of_listType "null: " >>= fun (s) ->
    return BoolType
  | Hd(e) ->
    type_of_expr e >>= 
    arg_of_listType "hd: " >>= fun (s) ->
    return s
  | Tl(e) ->
    type_of_expr e >>= 
    arg_of_listType "tl: " >>= fun (s) ->
    return (ListType s)
  ...
  | EmptyTree(t) ->
    return (TreeType t)  
  | Node(de, le, re) ->
    type_of_expr de >>= fun t ->
    type_of_expr le >>= 
    arg_of_treeType "node: " >>= fun l ->
    type_of_expr re >>= 
    arg_of_treeType "node: " >>= fun r ->
    return (TreeType t)
  | NullT(t) ->
    type_of_expr t >>= 
    arg_of_treeType "nullT: " >>= fun s ->
    return BoolType
  | GetData(t) ->
    type_of_expr t >>= 
    arg_of_treeType "getdata: " >>= fun s ->
    return s
  | GetLST(t) ->
    type_of_expr t >>= 
    arg_of_treeType "getlst: " >>= fun s ->
    return (TreeType s)
  | GetRST(t) ->
    type_of_expr t >>= 
    arg_of_treeType "getrst: " >>= fun s ->
    return (TreeType s)
